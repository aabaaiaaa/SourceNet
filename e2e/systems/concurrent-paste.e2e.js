...
const openFileManager = async (page, instanceNumber = 1) => {
    ...
    // Wait for the expected number of File Manager windows
    const windows = page.locator('.window:has(.file-manager)');
    await expect(windows).toHaveCount(instanceNumber, { timeout: 5000 });

    return windows.nth(instanceNumber - 1);
};

/**
 * Select a file system in a File Manager window and wait for it to load
 */
const selectFileSystem = async (fileManagerWindow, fileSystemId) => {
    ...
    // Wait for file list to update (either show files or be empty but loaded)
    await expect(fileManagerWindow.locator('.file-list')).toBeVisible();
};
...
/**
 * Wait for a specific file to appear in a File Manager window
 */
const waitForFileToAppear = async (fileManagerWindow, fileName, timeout = 10000) => {
    await expect(fileManagerWindow.locator(`.file-item .file-name:text("${fileName}")`)).toBeVisible({ timeout });
};
...
test.describe('Concurrent Paste Operations', () => {
    ...
    test('should preserve files when two File Manager instances paste to same destination', async ({ page }) => {
        ...
        await expect(async () => {
            const connections = await page.evaluate(() => window.gameContext.activeConnections?.length || 0);
        ...
        await waitForFileToAppear(fm1, 'file_from_source_1.txt');

        await selectAllFilesInWindow(page, 1);
        await clickCopyInWindow(page, 1);

        // ========================================
        // STEP 2: Open second File Manager, select source 2
        // ========================================
        const fm2 = await openFileManager(page, 2);
        await selectFileSystem(fm2, 'fs-source-2');

        await waitForFileToAppear(fm2, 'file_from_source_2.txt');

        // ========================================
        // STEP 3: Re-copy from source 1 in FM1, switch to destination, paste
        // ========================================
        await selectFileSystem(fm1, 'fs-source-1');
        await waitForFileToAppear(fm1, 'file_from_source_1.txt');
        await selectAllFilesInWindow(page, 1);
        ...
        // Switch FM1 to destination and paste
        ...
        // ========================================
        // Speed up time to complete transfers faster
        ...
        // ========================================
        // Wait for both files to appear in the registry
        await expect(async () => {
            const registryState = await page.evaluate(() => {
                const fs = window.gameContext?.networkRegistry?.getFileSystem('fs-destination');
                return fs?.files?.map(f => f.name) || [];
            });
            expect(registryState).toContain('file_from_source_1.txt');
            expect(registryState).toContain('file_from_source_2.txt');
    ...
    test('should handle rapid sequential pastes from different sources to same destination', async ({ page }) => {
        ...
        // ========================================
        // STEP 5: Wait for transfers to complete
        // ========================================
        // Speed up time to complete transfers faster
        await page.evaluate(() => window.gameContext.setSpecificTimeSpeed(100));
